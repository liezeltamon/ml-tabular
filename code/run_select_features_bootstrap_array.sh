...
# RUN_ID_BASE=prog_stage_selectkbest_p005 \
# TRAIN_PATH=/well/immune-rep/users/yfg436/git/sle/results/prediction/create_input_table/prog_stage/group_id_healthy_progb_progf_missingness0_minuniqueNone/train.csv \
# TEST_PATH= \
# TARGET_COLUMN=prog_stage \
# UNIVARIATE_METHOD=select_k_best \
# SKIP_CORRELATED_SELECTION=true \
# MIN_NON_MISSING_PER_CLASS=3 \
# sbatch --array=0-99 -J select_features_prog_stage_p005_nocorr -p short,long --mem=200G --output=logs/select_features/prog_stage_selectkbest_p005_no_correlated_selection/%x_%A_%a.log.out --error=logs/select_features/prog_stage_selectkbest_p005_no_correlated_selection/%x_%A_%a.log.err run_select_features_bootstrap_array.sh

# mkdir -p logs/select_features/progb_vs_nonprogb_selectkbest_p005
# RUN_ID_BASE=progb_vs_nonprogb_selectkbest_p005 \
...
# UNIVARIATE_METHOD=select_k_best \
# SKIP_CORRELATED_SELECTION=false \
# MIN_NON_MISSING_PER_CLASS=3 \
...
# SKIP_CORRELATED_SELECTION=true \
# MIN_NON_MISSING_PER_CLASS=3 \
# sbatch --array=0-99 -J select_features_bootstrap_p005_nocorr -p short,long --mem=200G --output=logs/select_features/progb_vs_nonprogb_selectkbest_p005_no_correlated_selection/%x_%A_%a.log.out --error=logs/select_features/progb_vs_nonprogb_selectkbest_p005_no_correlated_selection/%x_%A_%a.log.err run_select_features_bootstrap_array.sh

# mkdir -p logs/select_features/progb_vs_nonprogb_singlefeatureperformance_auc
# RUN_ID_BASE=progb_vs_nonprogb_singlefeatureperformance_auc \
# TRAIN_PATH=/well/immune-rep/users/yfg436/git/sle/results/prediction/create_input_table/group_id_nonprogb_progb_missingness0_minuniqueNone/train.csv \
# TEST_PATH=/well/immune-rep/users/yfg436/git/sle/results/prediction/create_input_table/group_id_nonprogb_progb_missingness0_minuniqueNone/test.csv \
# TARGET_COLUMN=is_progb \
# UNIVARIATE_METHOD=single_feature_performance \
# SKIP_CORRELATED_SELECTION=false \
# MIN_NON_MISSING_PER_CLASS=3 \
# sbatch --array=0-99 -J select_features_bootstrap_sfp_auc -p short,long --mem=200G --output=logs/select_features/progb_vs_nonprogb_singlefeatureperformance_auc/%x_%A_%a.log.out --error=logs/select_features/progb_vs_nonprogb_singlefeatureperformance_auc/%x_%A_%a.log.err run_select_features_bootstrap_array.sh
...
# SKIP_CORRELATED_SELECTION=true \
# MIN_NON_MISSING_PER_CLASS=3 \
# sbatch --array=0-99 -J select_features_bootstrap_sfp_auc_nocorr -p short,long --mem=200G --output=logs/select_features/progb_vs_nonprogb_singlefeatureperformance_auc_no_correlated_selection/%x_%A_%a.log.out --error=logs/select_features/progb_vs_nonprogb_singlefeatureperformance_auc_no_correlated_selection/%x_%A_%a.log.err run_select_features_bootstrap_array.sh
...
: "${RUN_ID_BASE:?RUN_ID_BASE must be set by launcher/env}"
: "${TRAIN_PATH:?TRAIN_PATH must be set by launcher/env}"
: "${TARGET_COLUMN:?TARGET_COLUMN must be set by launcher/env}"
...
if [[ "${SKIP_CORRELATED_SELECTION}" == "true" ]]; then
    RUN_ID="${RUN_ID:-${RUN_ID_BASE}_no_correlated_selection}"
